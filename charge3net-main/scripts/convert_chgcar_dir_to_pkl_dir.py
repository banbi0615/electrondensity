...
from convert_chgcar_to_pkl import convert as chgcar_to_npypkl
from write_mp_probe_count_file import count_elements_in_numpy_files
from write_dummy_split import main as write_split
...
parser.add_argument("--input", type=str, default="D:\\QM9\\QM9_10files", help="Directory containing **/CHGCAR files")
parser.add_argument("--output", type=str, default="D:\\QM9\\qm9", help="Directory to save .npy and .pkl files to")
...
def convert(f, input_dir=None, output_dir=None, write_filelist=True, write_spin=False):
    ...
    fname = relative_path_parts[0].split('.CHGCAR')[0]
...
if __name__ == "__main__":
    ...
    print("Found files:", chgcar_files)
    
    filelist_file = output_dir / "filelist.txt"
    # 后面说没有filelist.txt这个文件，但是with_open不应该会自动创建这个文件吗
